...
# The cpu_usage_tool is the command and ~/backups/cpuuse/cpu_usage.txt is teh filepath
# In the above example, the cpu_usage_tool` command will output CPU usage information into a `cpu_usage.txt file.
# Do not forget to use the -h option for free memory, dis usage, and free disk space

# Free memory output to a free _mem.txt file
